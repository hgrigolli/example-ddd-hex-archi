...
class OnHoldFlightUseCaseTest extends UseCaseTest {

    @InjectMocks
    private DefaultOnHoldFlightUseCase useCase;
    ...
    @Test
    void givenAValidCommand_whenCallExecute_thenFlightShouldBeOnHold() {
        // given
        ...
        final var expectedOnHold = LocalDateTime.of(2024, 1, 14, 22, 0);

        flight.board();

        when(flightGateway.findById(expectedFlightID)).thenReturn(java.util.Optional.of(flight));

        final var command = new OnHoldFlightCommand(expectedFlightID.getValue(), expectedOnHold);

        // when
        Assertions.assertDoesNotThrow(() -> useCase.execute(command));
        Mockito.verify(flightGateway, Mockito.times(1)).update(
                Mockito.argThat(aFlight -> aFlight.getOnHoldUntil().isEqual(expectedOnHold))
        );
    }
